...
class EpubListScreen extends StatelessWidget {
  const EpubListScreen({Key? key, required this.mapPercentDownload, required this.mapPercentDelete, required this.data})
      : super(key: key);
        ...
        return GestureDetector(
          onTap: () {
            ...
            Navigator.push(
              _,
              MaterialPageRoute(
                  builder: (context) => _EpubViewReader(
                        epubFromAsset: data[index].title ?? "",
                        epubFromFile: (data[index].isDownloaded ?? false) ? data[index].file : null,
                      )),
            );
          },
          child: EpubBookCardView(
            ...
            isDeleting: mapPercentDelete.keys.first == data[index].id! && mapPercentDelete.values.first > 0,
            onDownload: () {
